...
NSString * const YSMenuWillAppearNotification = @"YSMenuWillAppearNotification";
NSString * const YSMenuDidAppearNotification = @"YSMenuDidAppearNotification";
NSString * const YSMenuWillDisappearNotification = @"YSMenuWillDisappearNotification";
NSString * const YSMenuDidDisappearNotification = @"YSMenuDidDisappearNotification";
...
/// 背景色效果
static YSMenuBackgrounColorEffect   gBackgroundColorEffect = YSMenuBackgrounColorEffectSolid;
/// 是否显示阴影
...
typedef enum {
    ...
    YSMenuViewArrowDirectionNone,
    YSMenuViewArrowDirectionUp,
    YSMenuViewArrowDirectionDown,
    YSMenuViewArrowDirectionLeft,
    YSMenuViewArrowDirectionRight,

} YSMenuViewArrowDirection;


@interface YSMenuView : UIView
...
@interface YSMenuOverlay : UIView
...
#pragma mark - YSMenuOverlay

@implementation YSMenuOverlay
...
- (void)singleTap:(UITapGestureRecognizer *)recognizer {
    for (UIView *view in self.subviews) {
        if ([view isKindOfClass:[YSMenuView class]] && [view respondsToSelector:@selector(dismissMenu:)]) {
...
#pragma mark - YSMenuView
@implementation YSMenuView {
    YSMenuViewArrowDirection _arrowDirection;
    CGFloat _arrowPosition;
    UIView *_contentView;
    NSArray *_menuItems;
    YSMenuSelectedItem _selectedItem;
}
...
- (id)init {
    ...
    if(self) {
        ...
        if ([YSMenu hasShadow]) {
            ...
            self.layer.shadowColor = [YSMenu tintColor].CGColor;
...
- (void)setupFrameInView:(UIView *)view fromRect:(CGRect)fromRect {
    ...
    const CGFloat widthPlusArrow = contentSize.width + [YSMenu arrowSize];
    const CGFloat heightPlusArrow = contentSize.height + [YSMenu arrowSize];
    ...
    if (heightPlusArrow < (outerHeight - rectY1)) {
        ...
        _arrowDirection = YSMenuViewArrowDirectionUp;
        ...
        _contentView.frame = (CGRect){0, [YSMenu arrowSize], contentSize};

        self.frame = (CGRect) {

            point,
            contentSize.width,
            contentSize.height + [YSMenu arrowSize]
        };

    } else if (heightPlusArrow < rectY0) {

        _arrowDirection = YSMenuViewArrowDirectionDown;
        CGPoint point = (CGPoint){
            rectXM - widthHalf,
        ...
        _arrowDirection = YSMenuViewArrowDirectionLeft;
        CGPoint point = (CGPoint){
            rectX1,
            rectYM - heightHalf
        };
        ...
        _contentView.frame = (CGRect){[YSMenu arrowSize], 0, contentSize};

        self.frame = (CGRect) {

            point,
            contentSize.width + [YSMenu arrowSize],
            contentSize.height
        };

    } else if (widthPlusArrow < rectX0) {

        _arrowDirection = YSMenuViewArrowDirectionRight;
        CGPoint point = (CGPoint){
        ...
        _arrowDirection = YSMenuViewArrowDirectionNone;
...
- (void)showMenuInView:(UIView *)view fromRect:(CGRect)rect menuItems:(NSArray *)menuItems selected:(YSMenuSelectedItem)selectedItem {
    ...
    YSMenuOverlay *overlay = [[YSMenuOverlay alloc] initWithFrame:view.bounds];
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:YSMenuWillAppearNotification object:nil];
    ...
    [UIView animateWithDuration:0.2 animations:^(void) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:YSMenuDidAppearNotification object:nil];
    }];
}

- (void)dismissMenu:(BOOL)animated {
    if (self.superview) {
        [[NSNotificationCenter defaultCenter] postNotificationName:YSMenuWillDisappearNotification object:nil];

        __weak typeof(self) weakSelf = self;
        void (^removeView)(void) = ^(void) {
            if ([weakSelf.superview isKindOfClass:[YSMenuOverlay class]])
                [weakSelf.superview removeFromSuperview];
            [weakSelf removeFromSuperview];

            [[NSNotificationCenter defaultCenter] postNotificationName:YSMenuDidDisappearNotification object:nil];
        };
    ...
    [YSMenu sharedMenu].isShow = NO;
}

- (void)performAction:(id)sender {
    [self dismissMenu:YES];
    UIButton *button = (UIButton *)sender;
    YSMenuItem *menuItem = _menuItems[button.tag];
...
- (UIView *)mkContentView {
    ...
    UIFont *titleFont = [YSMenu titleFont];

    CGFloat maxImageWidth = 0;
    CGFloat maxItemHeight = 0;
    CGFloat maxItemWidth = 0;

    for (YSMenuItem *menuItem in _menuItems) {
    ...
    for (YSMenuItem *menuItem in _menuItems) {
        ...
        const CGFloat itemHeight = MAX(titleSize.height, imageSize.height) + [YSMenu menuItemMarginY] * 2;
    ...
    UIImage *selectedImage = [YSMenuView selectedImage:(CGSize){maxItemWidth, maxItemHeight}];

    UIImage *gradientLine = [YSMenuView gradientLineWithColor:[YSMenu separatorColor]];
    ...
    for (YSMenuItem *menuItem in _menuItems) {
        ...
        if (menuItem.title.length) {
            ...
            if (!menuItem.enabled && !menuItem.image) {
                ...
                titleFrame = (CGRect){
                    kMarginX * 2,
                    [YSMenu menuItemMarginY],
                    maxItemWidth - kMarginX * 4,
                    maxItemHeight - [YSMenu menuItemMarginY] * 2
                };
        ...
        if (menuItem.image) {
            ...
            const CGRect imageFrame = {kMarginX * 2, [YSMenu menuItemMarginY], maxImageWidth, maxItemHeight - [YSMenu menuItemMarginY] * 2};
...
- (CGPoint)arrowPoint {
    ...
    if (_arrowDirection == YSMenuViewArrowDirectionUp) {

        point = (CGPoint){ CGRectGetMinX(self.frame) + _arrowPosition, CGRectGetMinY(self.frame) };

    } else if (_arrowDirection == YSMenuViewArrowDirectionDown) {

        point = (CGPoint){ CGRectGetMinX(self.frame) + _arrowPosition, CGRectGetMaxY(self.frame) };

    } else if (_arrowDirection == YSMenuViewArrowDirectionLeft) {

        point = (CGPoint){ CGRectGetMinX(self.frame), CGRectGetMinY(self.frame) + _arrowPosition  };

    } else if (_arrowDirection == YSMenuViewArrowDirectionRight) {
...
+ (UIImage *)selectedImage:(CGSize)size {
    ...
    CGContextSetFillColorWithColor(context, [[YSMenu selectedColor] CGColor]);
...
- (void)drawBackground:(CGRect)frame inContext:(CGContextRef) context {
    ...
    UIColor *tintColor = [YSMenu tintColor];
    if (tintColor) {
        CGFloat a;
    ...
    if ([YSMenu backgrounColorEffect] == YSMenuBackgrounColorEffectGradient) {
        R1-=0.2;
        G1-=0.2;
        B1-=0.2;
    }
    ...
    if (_arrowDirection == YSMenuViewArrowDirectionUp) {
        ...
        const CGFloat arrowX0 = arrowXM - [YSMenu arrowSize];
        const CGFloat arrowX1 = arrowXM + [YSMenu arrowSize];
        const CGFloat arrowY0 = Y0;
        const CGFloat arrowY1 = Y0 + [YSMenu arrowSize] + kEmbedFix;
        ...
        const CGFloat arrowX0 = arrowXM - [YSMenu arrowSize];
        const CGFloat arrowX1 = arrowXM + [YSMenu arrowSize];
        const CGFloat arrowY0 = Y1 - [YSMenu arrowSize] - kEmbedFix;
        ...
        const CGFloat arrowX1 = X0 + [YSMenu arrowSize] + kEmbedFix;
        const CGFloat arrowY0 = arrowYM - [YSMenu arrowSize];;
        const CGFloat arrowY1 = arrowYM + [YSMenu arrowSize];
        ...
        const CGFloat arrowX1 = X1 - [YSMenu arrowSize] - kEmbedFix;
        const CGFloat arrowY0 = arrowYM - [YSMenu arrowSize];;
        const CGFloat arrowY1 = arrowYM + [YSMenu arrowSize];
    ...
    UIBezierPath *borderPath = [UIBezierPath bezierPathWithRoundedRect:bodyFrame
                                                          cornerRadius:[YSMenu cornerRadius]];
    ...
    if (_arrowDirection == YSMenuViewArrowDirectionLeft ||
        _arrowDirection == YSMenuViewArrowDirectionRight) {
...
@implementation YSMenu {
    YSMenuView *_menuView;
    BOOL         _observing;
}
...
+ (void)reset {
    ...
    gBackgroundColorEffect = YSMenuBackgrounColorEffectSolid;
    gHasShadow = NO;
...
- (void)showMenuInView:(UIView *)view fromRect:(CGRect)rect menuItems:(NSArray *)menuItems selected:(YSMenuSelectedItem)selectedItem {
    ...
    // 创建MenuView
    _menuView = [[YSMenuView alloc] init];
